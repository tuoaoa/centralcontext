...
## 2026-06-02
* **21:07 (Antigravity)**: Resolved Chrome Extension Content Security Policy (CSP) inline script violations on ChatGPT/Gemini/Claude pages. Replaced inline script injection of `debugInject` with a dedicated `inject.js` content script running in the `MAIN` world, which registers the function in the page context cleanly and securely. Checked and pushed to main branch.
